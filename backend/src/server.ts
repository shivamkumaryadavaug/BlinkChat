...
async function bootstrap() {
  ...
  // throws on the X-Forwarded-For header and can break every request.
  app.set("trust proxy", 1);

  // TEMPORARY: log every incoming request so we can see in Render logs
  // whether requests are actually reaching the server. Remove once
  // the CORS/connectivity issue is confirmed fixed.
