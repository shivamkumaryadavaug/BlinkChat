...
export default function ChatRoom({ roomCode, password, onLeave, onExpired }: Props) {
  ...
  const typingTimeoutRef.currentRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  useEffect(() => {
    ...
    socket.emit("room:join", { roomCode, password, sessionId: savedSessionId }, (res) => {
      if (res.ok) {
  ...
  const handleDraftChange = (value: string) => {
    ...
    typingTimeoutRef.current = setTimeout(() => socket.emit("typing:stop", { roomCode }), 2000);
  };
